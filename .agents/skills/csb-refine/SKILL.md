...
4. **Rerun and compare.** Run the focused config, re-analyze, and classify the hypothesis as confirmed, weakened, split, blocked, or ruled out. If the result is diffuse, change the next config or monitor set; do not repeat the same run without a sharper purpose. If benchmark configuration has mutually exclusive monitors, make reruns with each of the monitors enabled once, to thus explore alternative hypotheses.
