...
const static char* UNIQUE_VAR(filenames)[1] = {"./#innodb_redo/#ib_redo594_tmp"};
const static int UNIQUE_VAR(num_filesizes) = 1;
const static uint64_t UNIQUE_VAR(filesizes)[1] = {536870912};
...
//    fd: fd_dir (resource)
//    file: ptr[in, buffer] {
//      buffer: {2e 2f 23 69 6e 6e 6f 64 62 5f 72 65 64 6f 2f 23 69 62 5f 72 65 64 6f 35 39 34 5f 74 6d 70 00} (length 0x1f)
//    }
//    flags: open_flags = 0x42 (4 bytes)
//    mode: open_mode = 0x1ff (2 bytes)
//  ]
//  returns fd
memcpy((void*)(0x20200c80ul+PTR_OFFSET), "./#innodb_redo/#ib_redo594_tmp\000", 31);
	res = syscall(__NR_openat, UNIQUE_VAR(ctx->dirfd), /*file=*/0x20200c80ul+PTR_OFFSET, /*flags=O_NONBLOCK|O_CREAT|O_RDWR*/0x842, /*mode=S_IXOTH|S_IWOTH|S_IROTH|S_IXGRP|S_IWGRP|S_IRGRP|S_IXUSR|S_IWUSR|0x100*/0x1ff);
...
//    }
//  ]
*(uint16_t*)(0x20200cc0ul+PTR_OFFSET) = 1;
*(uint16_t*)(0x20200cc2ul+PTR_OFFSET) = 0;
*(uint64_t*)(0x20200cc8ul+PTR_OFFSET) = 0;
*(uint64_t*)(0x20200cd0ul+PTR_OFFSET) = 0;
*(uint32_t*)(0x20200cd8ul+PTR_OFFSET) = 0;
	res = syscall(__NR_fcntl, /*fd=*/UNIQUE_VAR(ctx->r)[0], /*cmd=F_SETLK*/6ul, /*lock=*/0x20200cc0ul+PTR_OFFSET);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  fallocate arguments: [
//    fd: fd (resource)
//    mode: fallocate_mode = 0x10 (8 bytes)
//    off: intptr = 0x0 (8 bytes)
//    len: intptr = 0x20000000 (8 bytes)
//  ]
	res = syscall(__NR_fallocate, /*fd=*/UNIQUE_VAR(ctx->r)[0], /*mode=FALLOC_FL_ZERO_RANGE*/0x10ul, /*off=*/0ul, /*len=*/0x20000000ul);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  fsync arguments: [
//    fd: fd (resource)
//  ]
