...
const static char* UNIQUE_VAR(subdirs)[1] = {"#innodb_temp"};
const static int UNIQUE_VAR(num_filenames) = 1;
const static char* UNIQUE_VAR(filenames)[1] = {"./#innodb_temp/temp_10.ibt"};
const static int UNIQUE_VAR(num_filesizes) = 1;
const static uint64_t UNIQUE_VAR(filesizes)[1] = {81920};
...
memcpy((void*)(0x207c4b80ul+PTR_OFFSET), "./#innodb_temp/temp_10.ibt\000", 27);
	res = syscall(__NR_openat, UNIQUE_VAR(ctx->dirfd), /*file=*/0x207c4b80ul+PTR_OFFSET, /*flags=O_NONBLOCK|O_CREAT|O_RDWR*/0x842, /*mode=S_IXOTH|S_IWOTH|S_IROTH|S_IXGRP|S_IWGRP|S_IRGRP|S_IXUSR|S_IWUSR|0x100*/0x1ff);
...
*(uint32_t*)(0x207c4bd8ul+PTR_OFFSET) = 0;
	res = syscall(__NR_fcntl, /*fd=*/UNIQUE_VAR(ctx->r)[0], /*cmd=F_SETLK*/6ul, /*lock=*/0x207c4bc0ul+PTR_OFFSET);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  fallocate arguments: [
//    fd: fd (resource)
...
//  ]
	res = syscall(__NR_fallocate, /*fd=*/UNIQUE_VAR(ctx->r)[0], /*mode=*/0ul, /*off=*/0ul, /*len=*/0x14000ul);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  fallocate arguments: [
//    fd: fd (resource)
...
memcpy((void*)(0x207c9680ul+PTR_OFFSET), "./#innodb_temp/temp_10.ibt\000", 27);
	res = syscall(__NR_newfstatat, UNIQUE_VAR(ctx->dirfd), /*file=*/0x207c9680ul+PTR_OFFSET, /*statbuf=*/0x207c96c0ul+PTR_OFFSET, /*flag=*/0ul);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  close arguments: [
//    fd: fd (resource)
//  ]
