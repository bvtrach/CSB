...
//    }
//  ]
*(uint64_t*)(0x20fe6f00ul+PTR_OFFSET) = UNIQUE_VAR(ctx->r)[0];
*(uint64_t*)(0x20fe6f08ul+PTR_OFFSET) = 0x1e8480;
	res = syscall(__NR_clock_nanosleep, /*id=*/0ul, /*flags=*/0ul, /*rqtp=*/0x20fe6f00ul+PTR_OFFSET, /*rmtp=*/0x20fe6f40ul+PTR_OFFSET);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
	return 0;
}
