...
memcpy((void*)(0x201041c0ul+PTR_OFFSET), "/proc/meminfo\000", 14);
	res = syscall(__NR_openat, UNIQUE_VAR(ctx->dirfd), /*file=*/0x201041c0ul+PTR_OFFSET, /*flags=*/0x800, /*mode=*/0);
...
//  ]
	res = syscall(__NR_read, /*fd=*/UNIQUE_VAR(ctx->r)[0], /*buf=*/0x20104200ul+PTR_OFFSET, /*count=*/0x1ffful);
	if (res == -1 ) { assert(!abort_on_fail); UNIQUE_VAR(ctx->num_failed)++;} else {UNIQUE_VAR(ctx->num_succeeded)++;};
//  close arguments: [
//    fd: fd (resource)
//  ]
